...
export default function ProductDetails() {
  ...
  useEffect(() => {
    if (!product) {
      document.title = "Lynix Shop - Premium Deals";
      return;
    }
    
    // SEO Meta Updates
    document.title = `${product.name} | Buy Online in Sri Lanka | Lynix`;
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", `Buy ${product.name} for just Rs.${product.price.toLocaleString()} at Lynix Sri Lanka. Premium ${product.category} with fast delivery!`);
  ...
  const jsonLd = {
    ...
    "name": product.name,
    "image": allImages,
    "description": `Buy ${product.name} securely online at Lynix Shop Sri Lanka. Fast delivery.`,
    "sku": `LYN-${product.id}`,
    "offers": {
      "@type": "Offer",
      ...
      "seller": { "@type": "Organization", "name": "Lynix Shop Sri Lanka" }
    },
    "aggregateRating": {
      "@type": "AggregateRating",
      "ratingValue": product.rating || 4.5,
      "reviewCount": Math.floor(product.id * 3) + 12 // Safe determinism
    }
  };
